...
for i=1:length(c)
        for j=c{i}
            a=a+1; %Contador de bucles totales de los 2 comandos for.
            media_velocidades=sum(j)/length(j);
            desviacion_velocidades=std(j);
            valor_min_velocidades=min(j);
            valor_max_velocidades=max(j); 
            if a==1 %Este if se utiliza para cambiar los nombre de ruta y conductor en cada bucle.
                name='A-1';
                conductor=1;
            elseif a==2
                name='A-1';
